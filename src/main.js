...
function init()
{
   ...
   const sizes = {
    ...
    gui.add(parameters,"emissive_bust",1.0,10.0,0.1).name("Emissive Boost").onChange((value) => {
        emissive_bust.value = value;
    });
    
    gui.add(parameters,"noise_amp",0.0,3.0,0.01).name("Noise Amplitude").onChange((value) => {
        u_noise_amp.value = value;
    }); 

    gui.add(parameters,"explode_amp",0.1,2.5,0.01).name("Explode Amplitude").onChange((value) => {
        u_explode_amp.value = value;
    });

    gui.add(parameters,"spring",0.0,0.1,0.01).
    name("Spring").onChange((value) => {
        u_spring.value = value;
    });
    gui.add(parameters,"friction",0.0,0.99,0.001
    ).name("Friction").onChange((value) => {
